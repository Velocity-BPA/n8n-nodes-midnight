...
export const description: INodeProperties[] = [
  {
    ...
    description: `Maximum number of blocks to return (max: ${PAGINATION.MAX_LIMIT})`,
    typeOptions: {
      minValue: 1,
...
export async function execute(
  ...
  const response = await midnightGraphQLRequest.call(this, blockQueries.list, pagination);

  const data = (response as { blocks: unknown[] }).blocks;
