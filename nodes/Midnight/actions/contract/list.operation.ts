...
export const description: INodeProperties[] = [
  {
    ...
    description: `Maximum number of contracts to return (max: ${PAGINATION.MAX_LIMIT})`,
    typeOptions: {
      minValue: 1,
...
export async function execute(
  ...
  const response = await midnightGraphQLRequest.call(this, contractQueries.list, pagination);

  const data = (response as { contracts: unknown[] }).contracts;
